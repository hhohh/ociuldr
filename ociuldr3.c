...
sword getColumns(FILE *fpctl,OCIStmt *stmhp, struct COLUMN *collist)
{
  ...
  for (col = 0; col < numcols; col++)
  {
      ...
      switch(nextcol->coltype)
      {
          ...
          case SQLT_BLOB: /* BLOB */
             ...
             {	
                ...
                fprintf(fpctl,"  %s LOBFILE(lobfile_col%d) TERMINATED BY EOF NULLIF lobfile_col%d = 'NONE' ",nextcol->colname, col+1,col+1);
             }   
          	 break;
          case SQLT_CLOB: /* BLOB */
          	 DEFAULT_ARRAY_SIZE = 1;
    ...
    //printf("colname: %s type %d\n",nextcol->colname,nextcol->coltype);
    //define output
    switch(nextcol->coltype)
    {
      case SQLT_DATE:
  ...
  for (;;)
  {
    ...
    for(r=0;r<rows;r++)
    {
       for(c=0;c<colcount;c++)
       {
          if (*(cols[c]->indp+r) >= 0)
          {
            switch (cols[c]->coltype)
            {
              case 24: //LONG RAW not long type
